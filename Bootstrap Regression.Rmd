...
WildObj <- wild.boot(sample_data$y~sample_data$x, B=100) #perform the wild bootstrap


```

```{r}
# 1. Run the regression and obtain the estimators and residuals for Wild Bootstrap


wild_boot <- function(sample_data, sample_model) {
 
  # 2. Resample the residuals with replacement and obtain the bootstrapped residual vector resb
  
  normal_vector <- rnorm(nrow(sample_data))
  # 3. Obtain the bootstrapped dependent variable by multiplying the estimator(s) from (1) with the original regressors and adding the bootstrapped residual
  yb <- sample_model$fitted.values + sample_model$residuals * normal_vector
  
  # 4. Run the regression with the bootstrapped dependent variables and the original regressors
  #wild_b <- lm(yb ~ sample_data$x)
  wild_b <- update(sample_model, yb ~ ., data=sample_data)
  
  unname(wild_b$coefficients)
}


beta_boot_wild <- replicate(1000, wild_boot(sample_data, sample_model))
