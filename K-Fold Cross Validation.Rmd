...
Evenly sized groups:
```{r}
rows <- nrow(cars_data_clean)
repititions <- ceiling(rows / k)
groups <- rep(1:k, repitions)[1:rows]
scrambled_groups <- sample(groups, rows, replace = FALSE)
cars_data_clean_k$my.folds <- scrambled_groups
```
