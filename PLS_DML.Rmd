...
for(i in 1:M){
  ...
  z= pls_variable[,"REP"] + pls_variable[,"INV"]# = Generate z = #
  ...
  # = Naive DML = #
  # = Compute ghat = #
  model=randomForest(as.matrix(z),y)
  ...
  z_mat = as.matrix(z)
  model1=randomForest(as.matrix(z_mat[IC,]),y[IC])
  model2=randomForest(as.matrix(z_mat[I,]),y[I])
  G1=predict(model1,as.matrix(z_mat[I,]))
  G2=predict(model2,as.matrix(z_mat[IC,]))
 
  # = Compute mhat and vhat on both samples = #
  modeld1=randomForest(as.matrix(z_mat[IC,]),d[IC])
  modeld2=randomForest(as.matrix(z_mat[I,]),d[I])
