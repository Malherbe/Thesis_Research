...
naive_dml <- function(z, d, y) {
  # ...
  # should return theta estimate
}

naive_dml_theta <- naive_dml(z, sec, trust)
...
cf_dml <- function(cont_var, expl_var, outc_var){
  numb_var <- length(outc_var)
  I <- sort(sample(1:numb_var, numb_var/2))
  IC <- setdiff(1:numb_var, I)
  
  z_mat <- as.matrix(cont_var)
  model1 <- randomForest(as.matrix(z_mat[IC,]), outc_var[IC])
  model2 <- randomForest(as.matrix(z_mat[I,]), outc_var[I])
  G1 <- predict(model1, as.matrix(z_mat[I,]))
  G2 <- predict(model2, as.matrix(z_mat[IC,]))
  
  modeld1 <- randomForest(as.matrix(z_mat[IC,]), expl_var[IC])
  modeld2 <- randomForest(as.matrix(z_mat[I,]), expl_var[I])
  M1 <- predict(modeld1, as.matrix(z_mat[I,]))
  M2 <- predict(modeld2, as.matrix(z_mat[IC,]))
  V1 <- expl_var[I]-M1
  V2 <- expl_var[IC]-M2
  
  theta1 <- mean(V1*(trust[I]-G1))/mean(V1*expl_var[I])
  theta2 <- mean(V2*(trust[IC]-G2))/mean(V2*expl_var[IC])
  theta_cf <- mean(c(theta1,theta2))
  return(theta_cf)
}
```
...
```{r} 
mc_cf_dml <- function(k=100, cont_var, expl_var, outc_var) {
  mc_results <- replicate(k, cf_dml(cont_var, expl_var, outc_var))
...
# TODO:
4. Run MC-CF-DML data
  a. research a bit about mc methods (take notes and citations)
  b. using z constructs (1000 iterations)
  c. using z items (1000 iterations)
  d. visualize distribution of both runs
  e. understand the difference in estimations
