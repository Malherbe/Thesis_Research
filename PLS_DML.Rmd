...
cf_dml <- function(cont_var, expl_var, outc_var, bootstrap=FALSE){
  ...
  if (bootstrap == TRUE) {
    ...
    boot_i <- sample(numb_var, size = length(numb_var), replace = TRUE)
    # I  <- # use first half of row bootstrapped indices
    I <- boot_i[1:(length(boot_i)/2)]
    # IC <- # use second half of row bootstrapped indices
    IC <- setdiff(1:boot_i, I)
  } else {
    I  <- sort(sample(1:numb_var, numb_var/2)) # add randomness
...
# Using z constructs with Boostrapped MC-CF-DML
```{r}
set.seed(367387)
...
# Using z items with Boostrapped MC-CF-DML
```{r}
set.seed(367387)
...
# TODO:
5. Boostrapped MC-CFDML
  a. Run z-construct Boostrapped MC-CFDML
  b. Run z-item Boostrapped MC-CFDML
  c. Visualize results
